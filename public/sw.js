...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches
      ...
      .then((keys) =>
        Promise.all(
          keys.filter((k) => k !== CORE_CACHE).map((k) => caches.delete(k)),
        ),
      )
...
self.addEventListener("fetch", (event) => {
  ...
  // 开发调试请求（Turbopack chunk, HMR 等）直接走网络，不走缓存
  if (
    url.hostname === "localhost" ||
    url.hostname === "127.0.0.1" ||
    url.pathname.includes("_next/static/development") ||
    url.pathname.includes("_next/static/chunks/turbopack")
  ) {
    return;
  }
