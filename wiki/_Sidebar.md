### 📖 Navelix 知识库目录

- [[🏠 官方 Wiki 首页|Home]]
- [[🚀 快速入门与部署|Quick-Start-快速入门]]
- [[📖 功能使用全手册|User-Guide-功能使用指南]]
- [[🏗️ 系统架构与选型|Architecture-系统架构与技术选型]]
- [[🛡️ 安全机制与运维|Security-安全机制与运维规范]]
- [[🔌 REST API 开放接口|REST-API-开放接口文档]]
- [[❓ 常见问题 FAQ|FAQ-常见问题与故障排查]]

---

### 📌 快捷功能索引
- **📅 日程与待办**
  - [日历分工指南](User-Guide-功能使用指南#3-日历日程成员分工与智能顺延)
  - [逾期自动顺延 Rollover](REST-API-开放接口文档#35-逾期待办一键定时顺延-rollover-api)
  - [Apple/Google 日历订阅](REST-API-开放接口文档#36-icalendar-标准外部日历订阅-apple--google--outlook)
- **📊 项目管理**
  - [四维看板与甘特图](User-Guide-功能使用指南#2-项目管理与-21-天交互式甘特图)
  - [原子化里程碑 API](REST-API-开放接口文档#42-创建新项目-支持原子化里程碑待办同步)
- **🤖 AI 智能中枢**
  - [AI 项目智能拆解](User-Guide-功能使用指南#41-ai-项目智能拆解与分工)
  - [账号额度监控](User-Guide-功能使用指南#44-反重力--codex-账号额度监控)
- [[📱 移动端 PWA 与快捷采集|Mobile-移动端与快捷采集]]
- **💡 自动化实战案例**
  - [GitHub Actions CI 通知](REST-API-开放接口文档#案例-a-github-actions--docker-ci-自动化构建通知)
  - [iOS 快捷指令一键录入](REST-API-开放接口文档#案例-b-ios-快捷指令一键录入日程待办并指派负责人)
  - [NAS 定时任务脚本](REST-API-开放接口文档#案例-d-定时脚本每晚自动顺延未完成待办-cron-rollover)
